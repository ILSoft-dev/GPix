# CleanDrive Bot (multi-user, v4.1, Google Drive backend)
...
2. `/settings` → чекбоксы: чистить метаданные (по умолчанию ✅) и
   обезличивать имена файлов (по умолчанию ⬜) — настраивается один раз,
   применяется автоматически к каждой загрузке без лишних вопросов.
...
5. Грузит файлы по одному (если один не загрузился — остальные всё равно
   продолжают грузиться), присылает точный отчёт по именам и ссылку на
   папку — доступна кому угодно по ссылке, без входа в Google-аккаунт.
...
```
Если таблица уже существует с прошлой версии (на Яндексе) — просто добавь
недостающие колонки:
```sql
alter table disk_users
  add column if not exists clean_metadata boolean default true,
